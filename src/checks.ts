...
function checkSecrets(file: string, content: string) {
  ...
  if (file.includes(".env")) {
    issues.push(
      createIssue("high", file, ".env file detected. Make sure it is not committed"),
    )
  }
...
function checkAstroTemplateIdentifiers(file: string, content: string) {
  const issues: Issue[] = []
  if (!file.endsWith(".astro")) return issues
...
export function checkFile(file: string, content: string) {
  return [
