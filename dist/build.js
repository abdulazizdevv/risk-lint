...
function normalizeBuildFilePath(file) {
    ...
    if (sourceIndex >= 0)
        return normalized.slice(sourceIndex);
    return normalized;
}
function parseBuildLocations(output) {
    const locations = [];
...
function parseBuildMessage(output) {
    const lines = output
        ...
        .map((line) => line.trim())
        .filter(Boolean);
    return (lines.find((line) => buildErrorPattern.test(line)) ||
        lines.find((line) => /^Error:/i.test(line)) ||
        lines.find((line) => line.includes("[ERROR]")) ||
        "Project build failed");
}
function isVueStyleBuildError(output, file) {
    return (file.endsWith(".vue") &&
        /vue&type=style|root stylesheet|\[vite:css\]|\[sass\]/i.test(output));
}
async function mapVueStyleLocation(output, location) {
    if (!isVueStyleBuildError(output, location.file))
        return location;
    try {
...
export async function checkBuildSyntax(files) {
    ...
    try {
        ...
        const rawLocation = (changedFiles
